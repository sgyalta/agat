...
var setTranslate = function (translate, byController) {
  ...
  if (!params.virtualTranslate) {
    if (Support.transforms3d) { $wrapperEl.transform(("translate3d(" + x + "px, " + y  + "px, " + (z + 40)+ "px)")); }
...
var Coverflow = {
  setTranslate: function setTranslate() {
    ...
    for (var i = 0, length = slides.length; i < length; i += 1) {
      ...
      var rotateY = 0;//isHorizontal ? rotate * offsetMultiplier : 0;
      var rotateX = 0;//isHorizontal ? 0 : rotate * offsetMultiplier;
      // var rotateZ = 0
      var translateZ = -translate * Math.abs(offsetMultiplier) - 60;
      ...
      var slideTransform = "translate3d(" + translateX + "px," + translateY + "px," + translateZ + "px)  rotateX(" + 0 + "deg) rotateY(" + 1 + "deg)";

      $slideEl.transform(slideTransform);
      $slideEl[0].style.zIndex = -Math.abs(Math.round(offsetMultiplier)) + 1;
      if (params.slideShadows) {
		  var $shadowBeforeEl = isHorizontal ? $slideEl.find('.shadow-left') : $slideEl.find('.swiper-slide-shadow-top');
      var $shadowAfterEl = isHorizontal ? $slideEl.find('.shadow-right') : $slideEl.find('.swiper-slide-shadow-bottom');
        if ($shadowBeforeEl.length === 0) {
          $shadowBeforeEl = $$1(("<div class=\"shadow-" + (isHorizontal ? 'left' : 'top') + "\"></div>"));
          $slideEl.append($shadowBeforeEl);
		}
        if ($shadowBeforeEl.length) { $shadowBeforeEl[0].style.opacity = 0.6 ? offsetMultiplier : 0; }
        if ($shadowAfterEl.length) { $shadowAfterEl[0].style.opacity = 0.6 > 0 ? -offsetMultiplier : 0; }
      }
    }
